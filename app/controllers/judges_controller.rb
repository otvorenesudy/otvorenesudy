class JudgesController < SearchController
  def show
    ...
    results_2013 = Judge.search(params.except(keys - [:indicators_2013]).merge(indicators_2013: true))
    @facets_2013 = results_2013.facets
    @others_2013 = params[:name] ? results_2013.to_a.map(&:first) : []

    results_2015 = Judge.search(params.except(keys - [:indicators_2015]).merge(indicators_2015: true))
    @facets_2015 = results_2015.facets
    @others_2015 = params[:name] ? results_2015.to_a.map(&:first) : []

    results_2017 = Judge.search(params.except(keys - [:indicators_2017]).merge(indicators_2017: true))
