class Hearing < ActiveRecord::Base
  ...
  scope :during_employment,
        lambda { |employment|
          where(court_id: employment.court).joins(:judgings).merge(Judging.of_judge(employment.judge))
        }
  ...
  belongs_to :court
  belongs_to :original_court, class_name: :Court
  ...
  has_many :exact_judges,
           ...
           source: :judge,
           class_name: :Judge,
           conditions: Judging.exact_conditions,
  ...
  mapping do
    ...
    analyze :accusations,
            as: lambda { |h| h.accusations.map { |a| "#{a.value} #{a.paragraphs.pluck(:description).join ' '}" } }
  ...
  facets do
    ...
    facet :historical,
          ...
          field: :date,
          facet: :date,
          value: lambda { |facet| [Time.now..Time.parse('2038-01-19')] if facet.terms == false }
  ...
  def social_security_case?
    mapper = lambda { |association| association.loaded? ? association.map(&:name) : association.pluck(:name) }

    names = mapper.call(defendants) + mapper.call(opponents) + mapper.call(proposers)
    names.any? { |name| ActiveSupport::Inflector.transliterate(name).downcase.match(/socialna\s+poistovna/) }
  end
