class Judge
  module Indicators2017
    ...
    included do
      mapping do
        analyze :indicators_2017, as: lambda { |j|
          !!(j.indicators_2017&.statistical && j.indicators_2017&.numerical)
        }

        analyze :decree_agenda_2017, as: lambda { |j|
          if j.indicators_2017&.statistical
            {
    ...
    def self.load!
      ...
      CSV.read(Rails.root.join('data', 'judge-statistical-indicators-2017.csv'), col_sep: ',', headers: true).each do |line|
      ...
      CSV.read(Rails.root.join('data', 'judge-numerical-indicators-2017.csv'), col_sep: ',', headers: true).each do |line|
        judge = Judge.find_by_id(line[1].strip)
